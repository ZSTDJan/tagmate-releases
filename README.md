...
- **历史打标学习**：学习你过去如何给相似图片打标签。
- **标签语义增强**：判断画面与标签含义是否相符。
- **角色**：一套独立的标签体系和学习成果。内置角色偏建筑摄影实景，也可以导入、分享其他角色，或学习你自己的图库；每次启用一个。
- **组合角色**：从多个角色中挑选标签，组成一个新角色。
- **校正设置**：通过推荐范围、排序权重、互斥、降权和阈值微调结果。
...
TagMate 的推荐过程可以理解为“看图 + 学习惯 + 对语义 + 规则校正”：系统先分析图片画面，再结合你过去给相似图片打标签的方式，以及标签本身的含义，最后通过推荐范围、排序权重、互斥、降权和阈值等校正设置整理结果。它给出的不是一套固定的通用答案，而是尽量贴近你自己的标签体系和判断方式。
